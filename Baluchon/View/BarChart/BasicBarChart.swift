...
class BasicBarChart: UIView {
    ...
    var dataEntries: [BarEntry]? = nil {
        didSet {
            ...
            if let dataEntries = dataEntries {
                scrollView.contentSize = CGSize(width: (barWidth + space)*CGFloat(dataEntries.count),
                                                height: self.frame.size.height)
                mainLayer.frame = CGRect(x: 0, y: 0, width: scrollView.contentSize.width,
                                         height: scrollView.contentSize.height)

                drawHorizontalLines()

                for data in 0..<dataEntries.count {
                    showEntry(index: data, entry: dataEntries[data])
                }
            }
        }
    }
    ...
    private func showEntry(index: Int, entry: BarEntry) {
        ...
        drawTitle(xPos: xPos - space/2,
                  yPos: mainLayer.frame.height - bottomSpace + 10,
                  title: entry.title,
                  color: entry.color)
    }
